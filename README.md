...
### 最終版の実行ファイルとプロジェクトを提出
- 最終版データのビルドデータ、インストカードのデータ、Unityプロジェクトを提出
  - 最終版をビルドデータを `X:\2022年\ゲーム学科2\平井・田中・細谷\最新実行ファイル` フォルダー内の**作品名のフォルダー**に上書きコピーする
- ビルドフォルダーをZIP圧縮して、WebページのZIPを更新して、プルリクエストを作成する
- 以下の手順で展示版のUnityプロジェクトを提出
  ...
  - 圧縮したzipファイルを`X:\2022年\ゲーム学科2\平井・田中・細谷\最終プロジェクト` フォルダー内にコピー
...
## 後期8週目(12/7)
### 話題
- [Unityゲーム開発者ギルドアドベントカレンダー2022](https://adventar.org/calendars/7428)
- [Qiita. Unityアドベントカレンダー2022](https://qiita.com/advent-calendar/2022/unity)
...
- 展示データのビルドデータ、インストカードのデータ、Unityプロジェクトを提出
  - 展示したビルドデータを `X:\2022年\ゲーム学科2\平井・田中・細谷\DATフェスタ提出` フォルダー内に**作品名のフォルダーを作成して**コピーする
  - (担当がCGなら不要) インストカードのデータを作品名のフォルダー内にコピー
  - 以下の手順で展示版のUnityプロジェクトを提出
    - Unityプロジェクトをデスクトップにコピーする
    - デスクトップにコピーしたUnityプロジェクトフォルダーを開いて `.git`と`Library`フォルダーを削除する
    - デスクトップにコピーしたプロジェクトフォルダーを右クリックして 7-Zip > 圧縮を選んでzip形式で圧縮
    - 圧縮したzipファイルを作品名のフォルダーにコピー
- 展示をしていて気づいた点、来場者のコメントをチーム全員で話し合いながらGoogleドキュメントにまとめる
  - ファイルをコピーしている以外のメンバーがGoogleドキュメントを作成して、文章名を「DATフェスタまとめ」とする
  - 共有をクリックして、リンクを知っている全員が編集できるように設定して、共有リンクをSlackに貼り付ける
